 ...
 * the License.
 */
package mybatis.querymethods.spring.data.query.parser;
...
import mybatis.querymethods.spring.data.util.Assert;
import mybatis.querymethods.spring.data.util.StringUtils;

/**
 * Class to parse a {@link String} into a tree or {@link OrPart}s consisting of simple {@link Part}
 * instances in turn. Takes a domain class as well to validate that each of the {@link Part}s are
 * referring to a property of the domain class. The {@link PartTree} can then be used to build
 * queries based on its API instead of parsing the method name for each query execution.
 * 
...
public class PartTree implements Iterable<OrPart> {
   ...
   * @param domainClass the domain class to check individual parts against to ensure they refer to a
  ...
  /**
   * Return {@literal true} if the create {@link PartTree} is meant to be used for a query with
  ...
  /**
   * A part of the parsed source that results from splitting up the resource around {@literal Or}
   * keywords. Consists of {@link Part}s that have to be concatenated by {@literal And}.
   */
