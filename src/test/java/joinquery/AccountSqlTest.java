...
import static joinquery.QueryFunctions.avg;
import static joinquery.QueryFunctions.count;
import static joinquery.QueryFunctions.distinct;
import static joinquery.QueryFunctions.exist;
import static joinquery.QueryFunctions.max;
import static joinquery.QueryFunctions.noCondition;
import static joinquery.QueryFunctions.notExist;
import static joinquery.QueryFunctions.select;
import static joinquery.QueryFunctions.selectOne;
import java.util.Arrays;
import org.junit.Assert;
import org.junit.Test;
import joinquery.dialect.CommonsDialectImpl;
...
public class AccountSqlTest {
    ...
    public void testHavingSql() {
        ...
        String sql = dialect.buildSelectSql(queryWrapper);
        System.out.println(sql);
        Assert.assertEquals("SELECT * FROM `tb_account` GROUP BY `user_name` HAVING `age` BETWEEN  #{p1000000} AND  #{p1000001} ", sql);
    }
